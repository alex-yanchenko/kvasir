 ...
 *
 * `runChannel()` owns every side effect (open the db, bind the port, connect the
 * stdio transport) so importing this module — e.g. from the argv-routing entry
 * main.ts — has none; the compiled `kvasir channel` subcommand calls it.
 *
 * Config (env):
...
/** Start the channel: open durable state, bind the localhost bridge, register the
 * MCP tools, and connect the stdio transport. Never returns (the stdio transport
 * keeps the process alive) — the entry awaits it as the process's main line. */
  ...
  const server = new McpServer(
    { name: "kvasir", version: VERSION },
    {
      ...
      instructions: [
        ...
        "☐ 5. For EACH step: set highlight to 2-4 substrings copied VERBATIM (character-for-character, including indentation) from the changed ('+'/'-') lines of that step's file in the patch — this is how the step points at code. The server locates them in the diff and derives the exact line range and side for you, so do NOT read the @@ headers, count '+'/'-' lines, or open source files to find line numbers (that is the main thing that makes this slow). Take all of a step's substrings from ONE contiguous region (a single hunk) — the derived range spans the first region they match, so substrings scattered across different hunks would leave the rest unhighlighted; scope a step to one region and add a separate step for another. highlight is REQUIRED and must be found in the file's patch — publish REJECTS a step whose highlight isn't located (it would open to nothing in the panel), so copy the substrings exactly from the changed lines. Keep them short and distinctive; write body (a plain-language summary a codebase newcomer understands on first read — apply the PROSE RULE above, do NOT chase concision into bare identifiers) and a substantive detail where worthwhile; give 2-3 suggestion questions.",
