...
const server = new Server(
  { name: "pr-walkthrough", version: "0.1.0" },
  {
    ...
    instructions: [
      ...
      "☐ Walkthrough text (overview, step body/detail, answers, suggestions) is a user-facing artifact: write normal, full prose. Brevity/compression modes (e.g. caveman) do NOT apply — treat it like code or commit messages.",
      "",
      ...
      "☐ 3. Author the spec (shape in the publish_walkthrough description): set overview = 2-4 plain-text sentences (what it does, approach/architecture, key risks) — NOT shown as a step; it's chat background so a fresh session still understands the PR.",
      ...
      '☐ 5. If mode="incremental": author steps for ONLY what changed since the `since` commit, and publish a spec containing ONLY those new steps (do NOT re-include earlier steps).',
      "☐ 6. Call publish_walkthrough(spec). The Chrome extension renders it on the PR page.",
      "",
