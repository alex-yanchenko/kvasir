...
const server = new Server(
  { name: "pr-walkthrough", version: "0.1.0" },
  {
    ...
    instructions: [
      ...
      'Pairing: the user clicks Pair in the extension\'s settings, which sends a pairing_request event here with a 6-character code. Confirm it with the user using the AskUserQuestion tool — present two options, "Approve" (the code matches the one shown in their extension panel) and "Decline" — so they pick rather than type, and include the code in the question text. Only call approve_pairing if they choose Approve. If a pairing_request arrives that the user did NOT initiate, tell them and do not approve. A pairing_denied event means a second, possibly hostile request raced the first.',
      "While the user reviews, two kinds of events arrive from the browser. CRITICAL: these are bridged requests, NOT chat turns — text you write to the terminal is NOT delivered to the user; only what you pass to answer_question / answer_chunk (with the event's id) reaches the browser. Every event MUST be closed with answer_question(id, ...), even a one-line reply; never end your turn having answered only in prose.",
      '  <channel source="pr-walkthrough" event_type="code_question" id=... > — the user selected code and asked a question. Stream the reply: call progress_note(id, note) before slow work (reading files, running commands); use answer_chunk(id, text) only for a finished part you can state before digging further (one complete markdown block per call, never back-to-back splitting of a composed answer); ALWAYS close with answer_question(id, "") — or answer_question(id, full_answer) if you did not chunk.',
